...
export const updatePlaylist = async (req, res) => {
  ...
  try {
    // 1. Kiểm tra quyền sở hữu & Lấy thông tin cũ
    const [playlists] = await promiseDb.query("SELECT * FROM playlists WHERE id = ?", [playlist_id]);
    
    if (playlists.length === 0) return res.status(404).json({ message: "Không tìm thấy playlist" });
    
    const currentPlaylist = playlists[0];
    
    if (currentPlaylist.user_id !== user_id) {
        return res.status(403).json({ message: "Bạn không sở hữu playlist này" });
    }

    // 2. Logic lấy Thumbnail URL
    // Nếu có upload file mới -> lấy path từ Cloudinary
    // Nếu không -> giữ nguyên thumbnail cũ
    const thumbnail_url = req.file ? req.file.path : currentPlaylist.thumbnail_url;

    // 3. Chuẩn bị dữ liệu update
    // Dùng toán tử OR (||) để giữ giá trị cũ nếu người dùng không gửi lên (tùy logic frontend)
    // Ở đây ta giả định frontend gửi đầy đủ hoặc ta dùng giá trị cũ
    const newName = name || currentPlaylist.name;
    const newDesc = description !== undefined ? description : currentPlaylist.description;
    ...
    await promiseDb.query(query, [newName, newDesc, thumbnail_url, playlist_id]);

    res.json({ message: "Cập nhật playlist thành công!", thumbnail_url });
