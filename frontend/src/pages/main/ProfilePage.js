...
import { 
  FiEdit3, FiMapPin, FiCalendar, FiMail, FiPhone, FiMusic, FiUser, FiSettings 
} from "react-icons/fi";
        ...
        <div className="container mx-auto px-4 max-w-5xl">
          <div className="relative -mt-16 md:-mt-20 mb-6 flex flex-col md:flex-row items-center md:items-end gap-6">
            ...
            <div className="flex-1 text-center md:text-left mb-2 md:mb-[-15px]">
              <h1 className="text-3xl font-bold text-gray-800 mb-1">{profile.full_name || profile.username}</h1>
              <p className="text-gray-500 font-medium">@{profile.username}</p>
              
              {/* Stats nhỏ */}
              <div className="flex items-center justify-center md:justify-start gap-4 mt-3 text-sm text-gray-600">
                <span className="flex items-center gap-1"><FiMusic /> <strong>{playlists.length}</strong> Playlists</span>
                <span className="flex items-center gap-1"><FiCalendar /> Tham gia: {joinDate}</span>
              </div>
            </div>
        ...
        {isEditing ? (
          <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100 animate-fade-in">
            <div className="flex justify-between items-center mb-6 border-b pb-4">
               ...
               <button onClick={() => setIsEditing(false)} className="text-gray-500 hover:text-red-500">Đóng</button>
            </div>
            <ProfileForm
              user={profile}
          ...
          <>
            ...
            {activeTab === "playlists" && (
              <div className="animate-fade-in">
                <h3 className="text-xl font-bold text-gray-800 mb-4">Bộ sưu tập của tôi</h3>
                {playlists.length > 0 ? (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    {playlists.map((pl) => (
                      ...
                      >
                         ...
                         <h4 className="font-bold text-gray-800 truncate">{pl.name}</h4>
                         <p className="text-xs text-gray-500">{pl.description || "Không có mô tả"}</p>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="text-center py-16 bg-white rounded-xl border border-dashed border-gray-300">
                    <div className="text-gray-300 mb-4"><FiMusic size={48} className="mx-auto"/></div>
                    <p className="text-gray-500 text-lg">Bạn chưa tạo playlist nào.</p>
                    <p className="text-sm text-gray-400">Hãy bắt đầu tạo bộ sưu tập âm nhạc của riêng mình!</p>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === "about" && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
                 ...
                 <div className="md:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-gray-100 h-fit">
                    <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">Thông tin liên hệ</h3>
                    <div className="space-y-4">
                        <InfoRow icon={<FiMail/>} label="Email" value={profile.email} />
                        <InfoRow icon={<FiPhone/>} label="Số điện thoại" value={profile.phone || "Chưa cập nhật"} />
                        <InfoRow icon={<FiMapPin/>} label="Địa chỉ" value={profile.address || "Chưa cập nhật"} />
                        <InfoRow icon={<FiUser/>} label="Tuổi" value={profile.age || "Chưa cập nhật"} />
                    </div>
                 </div>

                 {/* Cột phải: Tóm tắt */}
                 <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-6 rounded-xl border border-blue-100 h-fit">
                    <h3 className="text-lg font-bold text-blue-800 mb-2">Giới thiệu</h3>
                    <p className="text-gray-600 text-sm leading-relaxed">
                       {profile.bio || "Người dùng này rất bí ẩn và chưa viết gì về bản thân."}
                    </p>
                    <div className="mt-6 pt-4 border-t border-blue-200">
                        <p className="text-xs text-blue-500 uppercase font-bold">Trạng thái tài khoản</p>
