// frontend/src/components/AudioPlayer.js (updated - reduced height)
import React, { useContext } from "react";
import { AudioContext } from "../context/AudioContext";

// 👇 1. Thêm hàm helper để format giây sang MM:SS
      ...
      <div className="flex items-center justify-between max-w-7xl mx-auto">
        <div className="flex items-center space-x-2">
          <button 
            ...
            className={`p-1 rounded-full ${currentIndex <= 0 ? 'text-gray-500 cursor-not-allowed' : 'text-white hover:bg-gray-800'}`}
          >
          ...
          <button 
            ...
            className={`p-1 rounded-full ${currentIndex >= currentPlaylist.length - 1 ? 'text-gray-500 cursor-not-allowed' : 'text-white hover:bg-gray-800'}`}
          >
            <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 13 20">
